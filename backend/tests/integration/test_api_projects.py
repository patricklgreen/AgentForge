...
class TestProjectCRUD:
    ...
    async def test_create_project_success(
        self, client: AsyncClient, sample_project_data: dict
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_project_rejects_missing_fields(self, client: AsyncClient):
        response = await client.post("/api/v1/projects/", json={"name": "Only name"})
        assert response.status_code == 422
    ...
    async def test_get_project_success(
        ...
        create = await client.post("/api/v1/projects/", json=sample_project_data)
...
class TestRunManagement:
    ...
    async def test_start_run_creates_running_run(
        self, client: AsyncClient, sample_project_data: dict, mock_orchestrator
    ):
        create = await client.post("/api/v1/projects/", json=sample_project_data)
        project_id = create.json()["id"]
        ...
        with patch(
            ...
            response = await client.post(f"/api/v1/projects/{project_id}/runs")
    ...
    @pytest.mark.asyncio
    async def test_start_run_on_nonexistent_project(self, client: AsyncClient):
        fake_id = str(uuid.uuid4())
    ...
    async def test_list_runs_for_project(
        ...
        with patch(
            ...
            await client.post(f"/api/v1/projects/{project_id}/runs")

        response = await client.get(f"/api/v1/projects/{project_id}/runs")
        assert response.status_code == 200
    ...
    async def test_get_run_with_events(
        ...
        with patch(
            ...
            run_resp = await client.post(f"/api/v1/projects/{project_id}/runs")
            run_id = run_resp.json()["id"]

        response = await client.get(f"/api/v1/projects/{project_id}/runs/{run_id}")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_get_nonexistent_run(
        ...
        response = await client.get(
            f"/api/v1/projects/{project_id}/runs/{fake_run_id}"
        )
        assert response.status_code == 404
